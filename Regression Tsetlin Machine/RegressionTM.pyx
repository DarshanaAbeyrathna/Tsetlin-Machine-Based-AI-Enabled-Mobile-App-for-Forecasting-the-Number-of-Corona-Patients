...
cdef class TsetlinMachine:
	...
	def __init__(self, number_of_clauses, number_of_features, number_of_states, s, threshold, max_target, min_target):
		...
		self.weights = np.zeros(shape=(self.number_of_clauses), dtype=np.int32)
	...
	def get_weights(self, int clause):
		return self.weights[clause]
...
#			print(self.weights[j])
			output_sum += self.clause_output[j]*self.clause_sign[j]*self.weights[j]
...
#		print(output_sum, y)
		...
		if y > output_value:
			# Deciding the feedbck to each clause, if "self.feedback_to_clauses[j]" = + its type I, if -, type II
			for j in xrange(self.number_of_clauses):
