...
df = np.loadtxt("Italy.txt").astype(dtype=np.float32)

labels = ['C(d-1)',	'C(d)',	'O(d-1)',	'O(d)',	'I(d-1)',	'I(d)']
...
save('TMextractsItaly.npy', TMextracts)
save('RegSupItaly.npy', suportive)
save('ThredsItaly.npy', thresholds)
save('LabsItaly.npy', labels)
