...
df = np.loadtxt("SKorea.txt").astype(dtype=np.float32)

labels = ['C(d-1)',	'C(d)',	'O(d-1)',	'O(d)',	'SK(d-1)',	'SK(d)']
...
save('TMextractsSkorea.npy', TMextracts)
save('RegSupSkorea.npy', suportive)
save('ThredsSkorea.npy', thresholds)
save('LabsSkorea.npy', labels)


for i in range(NOofTestingSamples):
    print ("out:", tsetlin_machine.predict(X_test[i]))
print ("MAPE on training data:", tsetlin_machine.evaluate(X_train, y_train, y_train.shape[0]))
print ("MAPE on test data:", tsetlin_machine.evaluate(X_test, y_test, y_test.shape[0]))
