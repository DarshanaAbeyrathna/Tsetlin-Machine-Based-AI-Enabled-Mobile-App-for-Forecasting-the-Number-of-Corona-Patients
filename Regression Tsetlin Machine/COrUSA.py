...
df = np.loadtxt("USA.txt").astype(dtype=np.float32)

labels = ['C(d-1)',	'C(d)',	'O(d-1)',	'O(d)',	'U(d-1)',	'U(d)']
...
save('TMextractsUSA.npy', TMextracts)
save('RegSupUSA.npy', suportive)
save('ThredsUSA.npy', thresholds)
save('LabsUSA.npy', labels)
